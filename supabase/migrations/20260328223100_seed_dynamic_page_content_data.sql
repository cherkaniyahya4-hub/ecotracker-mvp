...
begin
  ...
  if v_admin_id is not null then
    ...
    on conflict (user_id) do update set
      ...
      score_delta_text = excluded.score_delta_text,
      map_status = excluded.map_status,
      map_search_placeholder = excluded.map_search_placeholder,
      map_highlight = excluded.map_highlight,
      next_dropoff_label = excluded.next_dropoff_label,
      next_dropoff_value = excluded.next_dropoff_value,
      weekly_score_series = excluded.weekly_score_series,
      weekly_score_note = excluded.weekly_score_note,
      emission_series = excluded.emission_series,
      emission_note = excluded.emission_note,
      donut_metrics = excluded.donut_metrics,
      league_name = excluded.league_name,
      next_rank_text = excluded.next_rank_text,
      quick_actions_title = excluded.quick_actions_title,
      quick_actions = excluded.quick_actions,
      ai_tiles = excluded.ai_tiles,
      focus_tasks = excluded.focus_tasks,
      weekly_breakdown = excluded.weekly_breakdown,
      carbon_snapshot = excluded.carbon_snapshot,
      ecocoach_message = excluded.ecocoach_message,
    ...
    on conflict (user_id) do update set
      ...
      upcoming_challenge = excluded.upcoming_challenge,
      nudges = excluded.nudges,
      goal_health = excluded.goal_health,
      rewards_queue = excluded.rewards_queue,
      updated_at = timezone('utc', now());
    ...
    on conflict (user_id) do update set
      title = excluded.title,
      subtitle = excluded.subtitle,
      instant_guidance_title = excluded.instant_guidance_title,
      instant_guidance_subtitle = excluded.instant_guidance_subtitle,
      action_buttons = excluded.action_buttons,
      camera_title = excluded.camera_title,
      camera_subtitle = excluded.camera_subtitle,
      camera_status = excluded.camera_status,
      ai_waste_items = excluded.ai_waste_items,
      barcode_items = excluded.barcode_items,
      recent_scans_summary = excluded.recent_scans_summary,
      ecocoach_tip = excluded.ecocoach_tip,
      rewards_text = excluded.rewards_text,
      updated_at = timezone('utc', now());

    insert into public.scan_events (id, user_id, event_label, title, details)
    values
      ('44444444-4444-4444-8444-444444444445', v_admin_id, 'Yesterday', 'Plastic tray', 'Rinse and recycle bin.'),
      ('44444444-4444-4444-8444-444444444446', v_admin_id, 'Yesterday', 'Coffee grounds', 'Compost pickup scheduled.')
  ...
  if v_user_id is not null then
    ...
    insert into public.scan_events (id, user_id, event_label, title, details)
    values
      ('44444444-4444-4444-8444-444444444441', v_user_id, 'Yesterday', 'Plastic tray', 'Rinse and recycle bin.'),
      ('44444444-4444-4444-8444-444444444442', v_user_id, 'Yesterday', 'Coffee grounds', 'Compost pickup scheduled.'),
      ('44444444-4444-4444-8444-444444444443', v_user_id, '2 days ago', 'Aluminum cans', 'Logged for bonus streak.'),
      ('44444444-4444-4444-8444-444444444444', v_user_id, '2 days ago', 'Cleaner barcode', 'Refill station recommended.')
    on conflict (id) do update set
      event_label = excluded.event_label,
      title = excluded.title,
      details = excluded.details;
